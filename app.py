...
# CSS फिक्स जो चैट इनपुट को हमेशा नीचे विज़िबल (Visible) रखता है
st.markdown("""
<style>
...
/* FIX: Force bottom chat container to be visible and correctly colored at the bottom */
...
/* Padding to prevent chat input from overlapping the content */
...
# ऐप का टाइटल और टैगलाइन
st.title("PowerWise SDG 7")
st.markdown('<p class="unique-tagline">Your Affordable and Clean Energy Advisor</p>', unsafe_allow_html=True)
...
# --- 4. INITIALIZE PERSISTENT CHAT HISTORY & RENDERING ---
if "messages" not in st.session_state:
    st.session_state.messages = []

# पुराने चैट मैसेजेस को स्क्रीन पर रेंडर करना
for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        st.markdown(message["content"])

# चैट इनपुट बॉक्स (Searchbar) जो हमेशा स्क्रीन के सबसे नीचे रहेगा
if user_input := st.chat_input("Ask me about solar power or conservation..."):
    
    # 1. यूजर का मैसेज दिखाएं और सेव करें
    with st.chat_message("user"):
        st.markdown(user_input)
    st.session_state.messages.append({"role": "user", "content": user_input})
    
    # 2. बैकएंड से रिस्पांस जनरेट करें
    backend_response = run_api_backend(user_input, st.session_state.messages)
    response_text = backend_response["response"]
    
    # 3. बॉट (Assistant) का रिस्पांस दिखाएं और सेव करें
